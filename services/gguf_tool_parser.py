...
class GGUFToolCallParser:
    ...
    def __init__(self):
        ...
        self.fallback_patterns = [
            ...
            # CRITICAL: Handle malformed tool calls without parameters (like <tool_call>pwd</tool_call>)
            r'<tool_call>(\w+)</tool_call>',
            # Handle command-style tool calls (like <tool_call>ls -la</tool_call>)
            r'<tool_call>([a-zA-Z0-9_-]+(?:\s+[^<>]+)?)</tool_call>',
        ]
    ...
    def parse_response(self, response: str) -> List[ParsedToolCall]:
        ...
        if console:
            console.print(f"[cyan]Parsing response: {response[:100]}...[/]")
        
        tool_calls.extend(self._parse_with_pattern(response, self.tool_call_pattern))
        
        if console and tool_calls:
            console.print(f"[green]Primary pattern found {len(tool_calls)} tool calls[/]")
        
        # If no matches found, try fallback patterns
        if not tool_calls:
            if console:
                console.print(f"[yellow]No primary matches, trying {len(self.fallback_patterns)} fallback patterns[/]")
            ...
            for i, pattern in enumerate(self.fallback_patterns):
                ...
                if console and fallback_calls:
                    console.print(f"[green]Fallback pattern {i} found {len(fallback_calls)} tool calls[/]")
        ...
        for match in matches:
            try:
                if len(match.groups()) >= 2:
                    ...
                    # Handle single group patterns (malformed tool calls)
                    raw_content = match.group(1)
                    function_name, args_str = self._parse_malformed_tool_call(raw_content)
                ...
                try:
                    ...
                    if arguments is None:
                        arguments = {}  # Fallback to empty args
    ...
    def _parse_malformed_tool_call(self, raw_content: str) -> Tuple[str, str]:
        """
        Parse malformed tool calls and convert them to proper format.
        ...
        Args:
            raw_content: The raw content from malformed tool call
            
        Returns:
            Tuple of (function_name, json_args_string)
        """
        raw_content = raw_content.strip()
        
        # Check if it's a known tool name
        if self._known_tools and raw_content in self._known_tools:
            # Direct tool name match
            if raw_content == 'list_dir':
                return 'list_dir', '{"path": "."}'
            elif raw_content == 'read_file':
                return '', ''  # Can't fix without filename
            elif raw_content == 'write_file':
                return '', ''  # Can't fix without filename and content
            else:
                return raw_content, '{}'
        
        # Check if it looks like a shell command
        shell_command_indicators = [
            'pwd', 'ls', 'cd', 'mkdir', 'rm', 'cp', 'mv', 'cat', 'echo',
            'touch', 'find', 'grep', 'git', 'python', 'node', 'npm', 'pip'
        ]
        ...
        if parts:
            ...
            if base_command in shell_command_indicators or '/' in raw_content:
                return 'run_bash', json.dumps({"command": raw_content})
            
            # Check for common directory operations
            if base_command in ['list', 'show'] and ('files' in raw_content.lower() or 'dir' in raw_content.lower()):
                return 'list_dir', '{"path": "."}'
        
        # Fallback: treat as shell command if no specific match
