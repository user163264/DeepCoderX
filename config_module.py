...
class DeepCoderXConfig:
    ...
    def _load_gguf_settings(self):
        """Load GGUF model settings for direct inference - QWEN OPTIMIZED."""
        ...
        # QWEN MODEL PATH - Use found Qwen model directly
        ...
        if custom_path:
            ...
            # Use found Qwen model path
            qwen_model_path = Path("/Users/admin/Documents/MyProjects/Project_Genesis/models/qwen-coder/qwen2.5-coder-1.5b.gguf")
            if qwen_model_path.exists():
                self.GGUF_MODEL_PATH = qwen_model_path
                logger.info(f"Using found Qwen model: {self.GGUF_MODEL_PATH}")
            else:
                # Fallback search paths for GGUF models
                ...
                for search_path in search_paths:
                    ...
                    # Recursive search
                    for found_file in search_path.rglob("*qwen*.gguf"):
