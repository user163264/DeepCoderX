#!/usr/bin/env python3
"""
Test script to verify LM Studio endpoint connectivity and OpenAI compatibility.
...
def test_lm_studio_endpoint():
    """Test LM Studio endpoint connectivity at localhost:1234/v1"""
    console.print("[bold blue]Testing LM Studio Endpoint Connectivity[/]")
    console.print("=" * 50)
    ...
    try:
        ...
        if models_response.status_code == 200:
            console.print("[green]✓ LM Studio server is running[/]")
            models_data = models_response.json().get("data", [])
            console.print(f"[green]✓ Available models: {len(models_data)}[/]")
            for model in models_data[:3]:  # Show first 3 models
                console.print(f"  - {model.get('id', 'Unknown')}")
        else:
            console.print(f"[red]❌ LM Studio server responded with status {models_response.status_code}[/]")
            return False
    except requests.exceptions.RequestException as e:
        console.print(f"[red]❌ Cannot connect to LM Studio server: {e}[/]")
        console.print("[yellow]Make sure LM Studio is running and has a model loaded on localhost:1234[/]")
        return False
    
    # Test 2: Chat completions endpoint
    test_model = local_config["model"]
    ...
    try:
        ...
        if chat_response.status_code == 200:
            ...
            if chat_response.status_code == 404:
                console.print("[yellow]Hint: Make sure you have a model loaded in LM Studio[/]")
            return False
...
def test_openai_client_import():
    ...
    try:
        ...
        console.print("[green]✓ OpenAI client configured for LM Studio[/]")
        return True
...
def main():
    ...
    tests = [
        ("LM Studio Endpoint", test_lm_studio_endpoint),
    ...
    if all_passed:
        ...
        console.print("- Make sure LM Studio is running on localhost:1234")
        console.print("- Load a model in LM Studio (qwen2.5-coder-1.5b-instruct recommended)")
